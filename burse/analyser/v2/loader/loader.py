...
# форматы данных
data_formats = {'TICKER, PER, DATE, TIME, OPEN, HIGH, LOW, CLOSE, VOL': 1,
                'TICKER, PER, DATE, TIME, OPEN, HIGH, LOW, CLOSE': 2, 'TICKER, PER, DATE, TIME, CLOSE, VOL': 3,
                'TICKER, PER, DATE, TIME, CLOSE': 4, 'DATE, TIME, OPEN, HIGH, LOW, CLOSE, VOL': 5,
                'DATE, TIME, LAST, VOL, ID, OPER': 6}
# периодичность записей
time_periods = {'tick': 1, 'min': 2, '5 min': 3, '10 min': 4, '15 min': 5, '30 min': 6, '1 hour': 7, 'day': 8,
                'week': 9, 'month': 10}
...
class Loader:
    def __init__(self):
        # Необходимо, если изначально код будет запускаться из другой директории,
        # так как в этом случае корневой директорией будет та, в которой лежит изначально запускаемы .py - файл
        loader_info_path = inspect.getfile(self.__class__).replace('loader.py', 'loader_info.csv')
        self.info_df = pd.read_csv(loader_info_path, names=['tool_id', 'tool', 'market_id', 'market', 'ticker'])
    ...
    def get_info_by_ticker(self, ticker):
        """
        ticker подразумевается уникальным в рамках всего датафрейма!
        """
        result = self.info_df[self.info_df['ticker'] == ticker]
        if result.shape[0] == 0:
            raise Exception('ticker %s не указан в качестве ключа! Аварийное прекращение работы!' % ticker)
        if result.shape[0] > 1:
            raise Exception('ticker %s неуникален в рамках датафрейма! Аварийное прекращение работы!' % ticker)
        return result

    def download_quotations_for_name_and_period(self,
                                                name: 'название актива',
                                                new_data_path,
                                                start_date,
                                                end_date,
                                                period=time_periods['min']):
        info = self.get_info_by_ticker(name)
        market_id = info['market_id'].values[0]
        tool_id = info['tool_id'].values[0]
        self.download_quotations(name=name,
    ...
    @staticmethod
    def download_quotations(name: 'название актива; с этим названием будет сохранён файл',
                            new_data_path,
                            start_date,
                            end_date,
                            market_id,
        ...
        page = urllib.request.urlopen(
            'http://export.finam.ru/' + str(name) + '_' + start_date.strftime("%Y%m%d") + '_' + end_date.strftime(
                "%Y%m%d") + str(extension) + '?market=' + str(market_id) + '&em=' + str(tool_id) + '&code=' + str(
                name) + '&apply=0&df=' + str(start_date.day) + '&mf=' + str(start_date.month - 1) + '&yf=' + str(
                start_date.year) + '&from=' + str(start_date.strftime("%d.%m.%Y")) + '&dt=' + str(
                end_date.day) + '&mt=' + str(end_date.month - 1) + '&yt=' + str(end_date.year) + '&to=' + str(
                end_date.strftime("%d.%m.%Y")) + '&p=' + str(period) + '&f=' + str(name) + '_' + str(
                start_date.strftime("%y%m%d")) + '_' + str(end_date.strftime("%y%m%d")) + '&e=' + str(
                extension) + '&cn=' + str(name) + '&dtf=' + str(date_format) + '&tmf=' + str(
                time_format) + '&MSOR=' + str(time_from) + '&mstimever=' + str(time_zone) + '&sep=' + str(
                separator) + '&sep2=' + str(decimal_mark) + '&datf=' + str(data_format) + '&at=' + str(headers))
        # f = open(new_data_path + str(name) + '_' + start_date.strftime("%Y%m%d") + '_' + end_date.strftime(
        #         "%Y%m%d") + str(extension), "wb")
        f = open(new_data_path + str(name) + str(extension), "wb")
        content = page.read()
        print('http://export.finam.ru/' + str(name) + '_' + start_date.strftime("%Y%m%d") + '_' + end_date.strftime(
            "%Y%m%d") + str(extension) + '?market=' + str(market_id) + '&em=' + str(tool_id) + '&code=' + str(
            name) + '&apply=0&df=' + str(start_date.day) + '&mf=' + str(start_date.month - 1) + '&yf=' + str(
            start_date.year) + '&from=' + str(start_date.strftime("%d.%m.%Y")) + '&dt=' + str(
            end_date.day) + '&mt=' + str(end_date.month - 1) + '&yt=' + str(end_date.year) + '&to=' + str(
            end_date.strftime("%d.%m.%Y")) + '&p=' + str(period) + '&f=' + str(name) + '_' + str(
            start_date.strftime("%y%m%d")) + '_' + str(end_date.strftime("%y%m%d")) + '&e=' + str(
            extension) + '&cn=' + str(name) + '&dtf=' + str(date_format) + '&tmf=' + str(time_format) + '&MSOR=' + str(
            time_from) + '&mstimever=' + str(time_zone) + '&sep=' + str(separator) + '&sep2=' + str(
            decimal_mark) + '&datf=' + str(data_format) + '&at=' + str(headers))
        f.write(content)
        f.close()
